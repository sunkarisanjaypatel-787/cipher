...
# Initialization
print("[*] Initializing Cipher Core (External Storage Mode)...")

# Dynamic Path Detection for Logs
...
def get_battery():
    ...
    try:
        ...
        speak(f"Current power levels are at {data.get('percentage', 'Unknown')} percent.")
    except Exception:
        speak("Battery telemetry unavailable.")
...
def get_telemetry():
    try:
        ...
        return f"Telemetry offline: {e}"

def process_audio_stream(audio_bytes):
    """Sends in-memory audio bytes to Whisper API."""
...
def ask_cipher(query):
    ...
    if "diagnostics" in query or "telemetry" in query:
        context_block = f"CURRENT SYSTEM TELEMETRY:\n{get_telemetry()}\nDIRECTIVE: Translate this raw Linux telemetry into a smooth vocal readout."
    else:
        context_block = "DIRECTIVE: Answer the user's query directly and concisely."

    system_instruction = f"""You are Cipher, a highly advanced tactical digital assistant for a Solo Operative. 
...
def start_daemon():
    """Streaming Socket Server with Protocol Verification."""
    ...
    print("\n[*] ==========================================")
    print("[*] CIPHER STREAMING DAEMON ONLINE. PORT 9999")
    ...
    while True:
        try:
            ...
            log_speed("Connection Established.")
            ...
            if len(raw_data) == 7 and raw_data.decode('utf-8', errors='ignore') == "EXECUTE":
                log_speed("Legacy Ghost Ping detected. Ignoring.")
                continue

            log_speed(f"Stream Closed. Received {len(raw_data)} bytes.")

            if len(raw_data) > 100: # Ensure substantial audio was recorded
                wav_payload = create_wav_data(raw_data)
                command = process_audio_stream(wav_payload)
                
                if command and command.strip() != "":
                    if "battery" in command or "power" in command:
                        get_battery()
                    elif "status" in command or "online" in command:
